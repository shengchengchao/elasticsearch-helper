...
/**
 * @author huajiejun
 * @date 2020/6/12 4:34 下午
 */
public class EsDateHistogramResultMapper extends DefaultResultMapper {

    private List<AggDataDTO> resultList;

    private AggParamDTO aggParamDTO;

    public EsDateHistogramResultMapper(AggParamDTO aggParamDTO) {
    ...
    public <T> AggregatedPage<T> mapResults(SearchResponse response, Class<T> clazz, Pageable pageable) {
        ...
        ParsedDateHistogram parsedRange = aggregations.get(EsConstants.CODE);

        List<? extends Histogram.Bucket> aggregationBuckets = parsedRange.getBuckets();
