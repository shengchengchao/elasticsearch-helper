package com.xixi.search.elasticsearchhelper.index;

import com.google.common.collect.Lists;
import com.xixi.search.elasticsearchhelper.util.EsUtil;
...
public abstract class IndexOperate<T> {
    ...
    public IndexOperate() {
        ...
        if(StringUtils.isBlank(idField) && declaredFields!=null && declaredFields.length>0){
            ...
            if(StringUtils.isBlank(idField)){
                ...
                log.info(" IndexOperate.IndexOperate {}类--对应索引名{} 不存在id ",tClass.getName(),indexName);
            }
        }
    }
    ...
    protected void putMapping(String indexName){
        if(elasticsearchRestTemplate.indexExists(indexName)){
